from llm_plan_bench.models.prompt_generation import PromptGenerator, read_file, read_pddl_file
import argparse
import os
import json
...
(:action unstack
  ...
  (:goal (and (on b2 b3) (on b3 b1))))"""
    ...
    "reason": "Starting from initial state (b1 on b3 on b2), we need to completely disassemble the stack and rebuild it in the goal configuration (b2 on b3 on b1). The optimal plan requires 6 steps: first unstack b1 and put it down, then unstack b3 from b2, stack b3 on b1, and finally move b2 on top of b3.",
    "plan": "(unstack b1 b3)
(putdown b1)
(unstack b3 b2)
(stack b3 b1)
...
(stack b2 b3)"
    ...
    def forward(self) -> str:
        ...
        if self.use_one_shot:
            prompt = "This is an example thinking process:\n\n"
